...
namespace cfr_dynamics_ns {
    ...
    {
        // left X
        effective_c_r_.k_left_x =
        (cfr_model_.blade_rad_max - cfr_model_.blade_rad_min) /
        (cfr_model_.motors_limits.LXmin_abs - cfr_model_.motors_limits.LXmax);
        effective_c_r_.b_left_x =
        cfr_model_.blade_rad_max -
        effective_c_r_.k_left_x * cfr_model_.motors_limits.LXmin_abs;
        ...
        if (abs(model_inputs_.LXMotordeg) < cfr_model_.motors_limits.LXmin_abs)
            ...
            effective_c_r_.effective_rad_LX =
            effective_c_r_.k_left_x * abs(model_inputs_.LXMotordeg) +
            effective_c_r_.b_left_x;

        // right X
        effective_c_r_.k_right_x =
        (cfr_model_.blade_rad_max - cfr_model_.blade_rad_min) /
        (cfr_model_.motors_limits.RXmin_abs - cfr_model_.motors_limits.RXmax);
        effective_c_r_.b_right_x =
        cfr_model_.blade_rad_max -
        effective_c_r_.k_right_x * cfr_model_.motors_limits.RXmin_abs;
        ...
        if (abs(model_inputs_.RXMotordeg) < cfr_model_.motors_limits.RXmin_abs)
            ...
            effective_c_r_.effective_rad_RX =
            effective_c_r_.k_right_x * abs(model_inputs_.RXMotordeg) +
            effective_c_r_.b_right_x;

        // right Y
        effective_c_r_.k_right_y =
        (cfr_model_.blade_rad_max - cfr_model_.blade_rad_min) /
        (cfr_model_.motors_limits.RYmin_abs - cfr_model_.motors_limits.RYmax);
        effective_c_r_.b_right_y =
        cfr_model_.blade_rad_max -
        effective_c_r_.k_right_y * cfr_model_.motors_limits.RYmin_abs;
        if (abs(model_inputs_.RYMotordeg) < cfr_model_.motors_limits.RYmin_abs)
            ...
            if (model_inputs_.RYMotordeg >= 0)
                effective_c_r_.effective_rad_RY =
    ...
    void CfrDynamics::computeDynamics()
    {
        ...
        dynamics_gen_.Fy_l_rotor =
        model_params_.k_left * model_inputs_.NetTorque / effective_c_r_.effective_rad_LX;
        ...
        dynamics_gen_.Fy_r_rotor =
        model_params_.k_right * model_inputs_.NetTorque / effective_c_r_.effective_rad_RX;
        ...
        // friction force
        dynamics_gen_.F_friction =
        environment_params_.mu_1 * cfr_model_.mass * environment_params_.g;
        // drag force
        dynamics_gen_.F_drag_y = environment_params_.mu_3 * model_inputs_.vy;
        dynamics_gen_.Fy = dynamics_gen_.Fy_rotor - dynamics_gen_.F_friction -
                           dynamics_gen_.F_drag_y; // combined force in y direction
        dynamics_gen_.y_acc =
        dynamics_gen_.Fy / cfr_model_.mass; // acceleration in y direction

        // compute force generated by right rotor in x direction
        // dynamics_gen_.Fx_r_rotor = model_params_.k_right * model_inputs_.NetTorque /
        // effective_c_r_.effective_rad_RY - 61.967213;
        dynamics_gen_.Fx_r_rotor =
        model_params_.k_right * model_inputs_.NetTorque / effective_c_r_.effective_rad_RY;
        dynamics_gen_.F_drag_x = environment_params_.mu_3 * model_inputs_.vx;
        dynamics_gen_.Fx =
        dynamics_gen_.Fx_r_rotor - dynamics_gen_.F_friction - dynamics_gen_.F_drag_x;
        ...
        if (effective_c_r_.effective_rad_LX != std::numeric_limits<double>::infinity()) {
            if (model_inputs_.LXMotordeg > 0)
                dynamics_gen_.L_lever =
                cfr_model_.width / 2 + effective_c_r_.effective_rad_LX;
            else
                dynamics_gen_.L_lever =
                cfr_model_.width / 2 - effective_c_r_.effective_rad_LX;
        }
        ...
        if (effective_c_r_.effective_rad_RX != std::numeric_limits<double>::infinity()) {
            if (model_inputs_.RXMotordeg > 0)
                dynamics_gen_.R_lever =
                cfr_model_.width / 2 - effective_c_r_.effective_rad_RX;
            else
                dynamics_gen_.R_lever =
                cfr_model_.width / 2 + effective_c_r_.effective_rad_RX;
        }
        ...
        if (dynamics_gen_.rot_torque_left <= 0)
            dynamics_gen_.theta_acc_left =
            dynamics_gen_.rot_torque_left / cfr_model_.I; // CW
        else
            dynamics_gen_.theta_acc_left =
            dynamics_gen_.rot_torque_left / cfr_model_.I; // CCW

        if (dynamics_gen_.rot_torque_right <= 0)
            dynamics_gen_.theta_acc_right =
            -dynamics_gen_.rot_torque_right / cfr_model_.I; // CCW
        else
            dynamics_gen_.theta_acc_right =
            -dynamics_gen_.rot_torque_right / cfr_model_.I; // CW

        dynamics_gen_.theta_acc =
